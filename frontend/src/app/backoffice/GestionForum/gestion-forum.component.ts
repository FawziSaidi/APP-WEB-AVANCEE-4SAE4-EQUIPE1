...
export class GestionForumComponent implements OnInit {
  ...
  // ── Cache utilisateurs (enrichissement local) ─────────────────────
  userMap: Map<number, { id: number; name: string; lastName: string; email: string }> = new Map();
...
Keep it factual, clear and actionable. No bullet points, write in prose.`;
  ...
  loadData(): void {
    ...
    this.forumService.getAllPublications().subscribe({
      next: (pubs) => {
        ...
        // ✅ FIX: Enrichir les publications dont user est null via le cache local
        this.enrichPublicationsWithUsers();
        this.applyFilters();
        this.loading = false;
  ...
  /** Charge le cache utilisateurs puis enrichit les publications avec user=null */
  private enrichPublicationsWithUsers(): void {
    // Si le cache est déjà chargé, on enrichit directement
    if (this.userMap.size > 0) {
      this.applyUserEnrichment();
      return;
    }
    this.forumService.getUsersForCache().subscribe({
      next: (users) => {
        users.forEach(u => this.userMap.set(u.id, {
          id: u.id,
          name: u.name,
          lastName: u.lastName,
          email: u.email || ''
      ...
      error: () => this.applyUserEnrichment() // silencieux, getUserLabel() gère le fallback
    });
  }

  /** Applique l'enrichissement user sur toutes les publications dont user est null */
  private applyUserEnrichment(): void {
    this.publications.forEach(pub => {
      if (!pub.user) {
        const uid = pub.userId ?? (pub as any).userId;
        if (uid) {
  ...
  // ── GIF & mentions rendering ──────────────────────────────────────

  /** Extrait l'URL d'un GIF encodé sous la forme [GIF:url] dans le contenu */
  ...
  /** Retire le tag GIF du texte et rend les @mentions en HTML */
