...
export interface Publication {
  ...
  warningCount?: number;
  userId?: number;               // ✅ FIX: champ direct si user est null
...
/** DTO retourné par /users/all-for-cache */
...
export class ForumService {
  ...
  private readonly gatewayBase = 'http://localhost:8222';  // ✅ FIX: gateway au lieu de 8081 direct

  constructor(private http: HttpClient) {}

  // ── Users cache ───────────────────────────────────────────────────

  /** Charge tous les utilisateurs depuis user-service (même endpoint que commentaire-service) */
  getUsersForCache(): Observable<UserCacheDTO[]> {
    return this.http.get<UserCacheDTO[]>(`${this.gatewayBase}/users/all-for-cache`);  // ✅ via gateway
  }
