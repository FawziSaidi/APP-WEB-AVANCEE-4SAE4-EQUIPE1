...
export class CommentaireModalComponent implements OnInit {
  ...
  isPublicationOwner(): boolean {
    // ✅ FIX: fallback sur publication.userId si publication.user est null
    const ownerId = this.publication?.user?.id ?? this.publication?.userId;
    return Number(ownerId) === Number(this.currentUserId);
  }

  isPublicationAuthor(commentaireUserId?: number): boolean {
    // ✅ Vérifie si le commentateur est l'auteur de la publication
    const ownerId = this.publication?.user?.id ?? this.publication?.userId;
    return Number(commentaireUserId) === Number(ownerId);
  }
