...
/**
 * Initialise le cache local des utilisateurs au démarrage de reaction-service.
 *
 * Problème résolu :
 *   Les utilisateurs déjà existants en base n'ont jamais publié d'événement
 *   RabbitMQ → le cache UserCacheService est vide au démarrage → getSummary()
 *   affiche "User #3" au lieu du vrai nom dans la liste des reactors.
 *
 * Solution :
 *   Au démarrage, appel GET /users/all-for-cache sur user-service
 *   (endpoint public, sans token) pour charger TOUS les utilisateurs
